...
def generate_real_samples(n_samples):
    ...
    indices = np.random.choice(np.arange(y.size), replace=False, size=int(y.size * 0.1))
    y[indices] = 0
...
def define_generator(latent_dim,n_classes = 30):
        ...
        gen = Conv2DTranspose(128, (4, 3), strides=(4, 3), padding='same')(merge)
        gen = LeakyReLU(alpha=0.2)(gen)


        gen = Conv2DTranspose(512, (4, 3), strides=(2, 2), padding='same')(gen)
        gen = LeakyReLU(alpha=0.2)(gen)

        gen = Conv2DTranspose(256, (4, 3), strides=(2, 2), padding='same')(gen)
        gen = LeakyReLU(alpha=0.2)(gen)

        gen = Conv2DTranspose(128, (4, 3), strides=(2, 2), padding='same')(gen)
        gen = LeakyReLU(alpha=0.2)(gen)

        gen = Conv2DTranspose(64, (4, 3), strides=(2, 2), padding='same')(gen)
        gen = LeakyReLU(alpha=0.2)(gen)
        ...
        gen = Conv2DTranspose(64, (1, 1), strides=(1, 1), padding='same')(gen)
        gen = LeakyReLU(alpha=0.2)(gen)
...
def save_plot(data, epoch, n=4):
    aux = normalize(data[0])
    # plot images
    for i in range(n * n):
        plt.imshow(aux[i])
        plt.savefig("generated//generated_plot{}_{}.png".format(epoch + 1, i))

    plt.close()


# train the generator and discriminator
def train(g_model, d_model, gan_model, latent_dim, n_epochs=5000, n_batch=16):
    ...
    for i in tqdm(range(n_epochs)):
        ...
        for j in range(bat_per_epo):
            ...
            if (i + 1) % 100 == 0:
                ...
                x = list(range(1, len(final_d_loss) + 1))
